...
export default function JobCard({ job }: JobCardProps) {
  ...
  const getIcon = (label: string) => {
    ...
    if (
      ...
      label.toLowerCase().includes("on-site")
    )
  ...
  // ✅ Remove "Remote" tag if location already includes it
  const cleanTags = job.tags.filter(
    (tag) => !(job.location.toLowerCase().includes("remote") && tag.toLowerCase() === "remote")
  );

  // ✅ Combine & deduplicate tags
  const tagList = [...new Set([job.location, job.salary, ...cleanTags])];
    ...
    <Card className="rounded-xl border border-white/10 bg-white/[0.04] hover:bg-white/[0.06] transition-all duration-300 shadow-lg">
      <div className="p-6 sm:p-8 flex flex-col gap-5">
        ...
        <p className="text-sm text-foreground/70 leading-relaxed text-left">
          Lead strategy for {job.company}&apos;s infrastructure platform, driving scale, reliability,
          and developer productivity. Looking for a technical PM who thrives in cross-functional work
          and can balance long-term vision with iterative delivery.
        </p>
